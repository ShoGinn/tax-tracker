...
export const DashboardPage = () => {
  ...
  const summaryQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      const w2Gross = paychecks.reduce((total, paycheck) => total + parseDecimalString(paycheck.gross_wages), 0);
      const federalWithholding =
        paychecks.reduce((total, paycheck) => total + parseDecimalString(paycheck.federal_withholding), 0) +
        pensions.reduce((total, pension) => total + parseDecimalString(pension.federal_withholding), 0);
      const pensionGross = pensions.reduce((total, pension) => total + parseDecimalString(pension.gross_amount), 0);
      const nonTaxableGross = nonTaxableIncome.reduce((total, entry) => total + parseDecimalString(entry.amount), 0);
